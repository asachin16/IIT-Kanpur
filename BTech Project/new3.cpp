...
static void draw_subdiv_facet( IplImage* img, CvSubdiv2DEdge edge )
{
    ...
    if( i == count )
    {
        ...
        cvFillConvexPoly( img, buf, count, CV_RGB(rand()%266,rand()%266,rand()%266), CV_AA, 0 );
        cvPolyLine( img, &buf, &count, 1, 1, CV_RGB(0,0,0), 1, CV_AA, 0);
        draw_subdiv_point( img, pt->pt, CV_RGB(255,255,255));
    }
    free( buf );
}
...
static void run(int x[],int y[],int n)
{
    ...
    bkgnd_color = CV_RGB(rand()%266,rand()%266,rand()%266);
    ...
    for( i = 0; i < n; i++ )
    {
        CvPoint2D32f fp = cvPoint2D32f( x[i],
                                        y[i]);
...
int main()
{
    ...
    //correction_factor();
    image=cvLoadImage("D2-4-5000x_b.png",1);
	height=image->height;
	width=image->width;
	step=image->widthStep;
	channels=image->nChannels;
    ...
    grey=cvLoadImage("123.jpg",1);
    data1=(uchar *)grey->imageData;
    for(i=0;i<height;i++)
    {
	     ...
	     for(j=0;j<width;j++)
	     {
             ...
             if(h>200&&s>200&&v>200){
             	data1[i*step+j*channels+0]=255;
             	data1[i*step+j*channels+1]=255;
             	data1[i*step+j*channels+2]=255;
             }
		}
	}
   cvShowImage("image2",grey);
   cvSaveImage("abcd.jpg",grey);
