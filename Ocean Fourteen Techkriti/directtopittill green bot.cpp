...
int main()
{
    ...
    int key = 0;
   // com->sendChar('a');
    int pinkx,pinky,yellowx,yellowy,bluex,bluey,greeny,violetx,violety,botx,boty,i=0;
    int hb,sb,cb,wb;
    int greenx[21];
      ...
      // cvThreshold(img0, img0, 85, 255, CV_THRESH_BINARY);
        ...
        bottheta=atan((double)boty/(double)botx)*180/PI;
         if(pinky>giy && pinky<gfy){
                   if(greenx[0]-greenx[20]>0&&greenx>gix){
                                           com->sendChar('F');
                                           } 
                   else{
                                           com->sendChar('s');
