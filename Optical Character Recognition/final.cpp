...
{                                                             // templates are those images wid which we compare our images
...
int main()
{
   ...
   IplImage* char1temp[27];//declaring image array in which we give small letter templates
    ...
    int i;
    for(i=1;i<10;i++){
    num[i]=cvCreateImage(cvGetSize(numtemp[i]),IPL_DEPTH_8U,1);//creating image in order to make blob of the size of template
    }
    for(i=1;i<27;i++){
